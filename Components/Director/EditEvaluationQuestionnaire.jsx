...
const EditEvaluationQuestionnaire = ({ route, navigation }) => {
  ...
  const fetchQuestionnaire = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/Questionnaire/GetById/${questionnaireId}`
      );
  ...
  const toggleCritical = (index) => {
    ...
    updated[index].isCritical = !updated[index].isCritical;
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/Questionnaire/SaveAllChanges`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <ScrollView style={ss.container}>
      ...
      <Text style={ss.heading}>
        Edit Evaluation Questionnaire
      </Text>
      ...
      {/* ✅ NEW QUESTION CRITICAL CHECKBOX */}
      <TouchableOpacity
      ...
      {questions.map((q, index) => (
        ...
        <View
          key={q.id || index}
          style={ss.questionCard}
        >

          <View style={ss.circle}>
            <Text style={ss.circleText}>
              {index + 1}
            </Text>
          </View>
