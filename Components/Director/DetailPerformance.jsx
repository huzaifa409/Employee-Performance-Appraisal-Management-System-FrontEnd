...
const QuestionCard = ({ item, index }) => {
  ...
  const color = avgColor(item.AverageScore || 0);
    ...
    <View style={s.card}>
      ...
      <View style={s.avgRow}>
        ...
        >
          ...
          >
            Avg {(item.AverageScore || 0).toFixed(2)} / 4
          </Text>
        </View>
...
const SummaryStrip = ({ data }) => {
  ...
  const overallAvg =
    data.reduce(
      (s, d) => s + (d.AverageScore ?? 0),
      0
    ) / data.length;

  const totalResp = data.reduce(
    (s, d) =>
      s +
      (d.Score1 ?? 0) +
      (d.Score2 ?? 0) +
  ...
  const fetchDetails = async () => {
    try {
      ...
      if (
        ...
        const localData =
          await getConfidentialQuestionStats(
            teacherId,
            selectedSession,
            courseCode
          );
        ...
        {loading ? (
          ...
          <>
            <View style={s.listHeader}>
              <View
                style={s.listHeaderDot}
              />

              <Text
                style={s.listHeaderText}
              >
